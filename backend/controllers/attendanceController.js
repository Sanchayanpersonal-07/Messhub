...
export const markManualAttendance = async (req, res, next) => {
  try {
    ...
    // Manager এর জন্য time window নেই — manual override allowed
    ...
    if (existing)
      return res
        .status(400)
        .json({ msg: "Attendance already marked for this student" });
    ...
    res
      .status(201)
      .json({ msg: "Attendance marked manually", attendance: populated });
  } catch (err) {
    next(err);
  }
};
...
export const checkTodayAttendance = async (req, res, next) => {
  try {
    ...
    const marked = { breakfast: false, lunch: false, dinner: false };
    records.forEach((r) => {
      marked[r.meal_type] = true;
    });
    // ✅ Window info included — frontend এ Open/Closed দেখাতে কাজে লাগবে
    const windows = getAllMealWindows();
    res.json({ date: today, marked, windows });
  } catch (err) {
    next(err);
  }
};
