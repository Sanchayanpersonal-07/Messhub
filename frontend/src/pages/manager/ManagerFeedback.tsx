...
export default function ManagerFeedback() {
  ...
  const [editMap, setEditMap] = useState<
    Record<string, { status: Status; action_taken: string }>
  >({});
  ...
  const fetchAll = async () => {
    try {
      const res = await api.get<Feedback[]>("/manager/feedback");
      const data = res.data || [];
      setAllFeedback(data);
      const map: Record<string, { status: Status; action_taken: string }> = {};
      data.forEach((fb) => {
        map[fb._id] = {
          status: fb.status,
  ...
  const handleSave = async (id: string) => {
    ...
    try {
      setSavingId(id);
      const res = await api.put<Feedback>(`/manager/feedback/${id}/status`, {
        status: edit.status,
        action_taken: edit.action_taken || null,
      });
      setAllFeedback((prev) =>
        prev.map((fb) => (fb._id === id ? { ...fb, ...res.data } : fb)),
      );
  ...
  const setEdit = (
    id: string,
    patch: Partial<{ status: Status; action_taken: string }>,
  ) => setEditMap((prev) => ({ ...prev, [id]: { ...prev[id], ...patch } }));

  const isDirty = (fb: Feedback) => {
    const e = editMap[fb._id];
    if (!e) return false;
    return e.status !== fb.status || e.action_taken !== (fb.action_taken || "");
  };
  ...
  const pendingCount = allFeedback.filter(
    (f) => f.status !== "resolved",
  ).length;
  const highCount = allFeedback.filter((f) => f.priority === "high").length;
  const resolvedCount = allFeedback.filter(
    (f) => f.status === "resolved",
  ).length;
    ...
    <DashboardLayout>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {filtered.map((fb) => {
            const edit = editMap[fb._id] ?? {
              status: fb.status,
              action_taken: fb.action_taken || "",
            };
            const dirty = isDirty(fb);
            ...
            return (
              <Card
                ...
                style={{
                  borderLeft: `3px solid ${dirty ? "#f59e0b" : pc.cardBorder}`,
                }}
              >
                <CardContent className="py-4 px-4">
                  {dirty && (
                    <div
                      className="flex items-center gap-1.5 text-xs mb-3 px-2 py-1.5 rounded-lg"
                      style={{
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        >
                          ...
                          {fb.meal_id?.date
                            ? dayjs(fb.meal_id.date).format("DD MMM")
                            : "—"}
                        </span>
                        <span
                    ...
                    <div
                      className="flex flex-col gap-2 sm:w-52 shrink-0 rounded-xl p-3"
                      style={{
                    ...
                    >
                      ...
                      <Textarea
                        ...
                        rows={3}
                        placeholder="Add action note (visible to student & warden)..."
                        value={edit.action_taken}
                        onChange={(e) =>
                          setEdit(fb._id, { action_taken: e.target.value })
                        }
                      />

                      <Button
                        size="sm"
                        disabled={!dirty || savingId === fb._id}
